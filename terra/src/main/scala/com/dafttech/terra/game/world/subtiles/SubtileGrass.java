...
public class SubtileGrass extends Subtile {
    ...
    public void onTick(World world, float delta) {
        if (world.getTile(tile.getPosition().$minus(0, 1)).isOpaque()) {
            tile.removeSubtile(this);
        } else if (world.getTile(tile.getPosition().$minus(0, 1)).getTemperature() > 50 && !(this instanceof SubtileDryGrass)) {
            tile.removeSubtile(this);
    ...
    public void spread(World world) {
        Vector2i spreadPosition = tile.getPosition().$plus(new Random().nextInt(spreadDistance * 2) - spreadDistance,
                new Random().nextInt(spreadDistance * 2) - spreadDistance);
        Tile spreadTile = world.getTile(spreadPosition);
        if (spreadTile != null && spreadTile instanceof TileDirt && world.getTile(spreadPosition.$minus(0, 1)).isAir()) {
