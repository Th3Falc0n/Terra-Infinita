...
public class WeatherRainy extends Weather {
    ...
    public void update(World world, float delta) {
        Vector2 top = world.localPlayer().getPosition().$minus(0, Gdx.graphics.getHeight() / 2);

        if ((delta * 60f) * TerraInfinita.rnd().nextFloat() > 0.5f) {
            new ParticleRain(top.$plus((TerraInfinita.rnd().nextFloat() - 0.5f) * Gdx.graphics.getWidth(), 0), world).setVelocity(new Vector2(
