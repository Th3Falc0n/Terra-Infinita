    ...
    public void draw(AbstractScreen screen, Tile tile, Entity pointOfView, Object... rendererArguments) {
        ...
        if (tile instanceof ITileRenderRounded) {
            ...
            if (edgeTextures.length == 4) {
                boolean t = rTile.isFlatTo(tile.world, tile.getPosition().addY(-1));
                boolean b = rTile.isFlatTo(tile.world, tile.getPosition().addY(1));
                boolean l = rTile.isFlatTo(tile.world, tile.getPosition().addX(-1));
                boolean r = rTile.isFlatTo(tile.world, tile.getPosition().addX(1));
                if (t || l)
