...
public class TileLog extends Tile {
    ...
    public TileLog() {
        super();
        maxHeight = TerraInfinita.rnd.nextInt(10) + 5;
        maxWidth = TerraInfinita.rnd.nextInt(10) + 5;
    }
    ...
    public void onTick(World world) {
        super.onTick(world);
        if (living) growTree(world, TerraInfinita.rnd.nextInt(2));
    }

    public void growTree(World world, int num) {
        if (num == 0) growTree0(world);
        if (num == 1) growTree1(world);
    }

    public void growTree0(World world) {
        ...
        if (!grown) {
            ...
            if (height > maxHeight / 2.5f) {
                if (rnd.nextInt((int) ((float) Math.abs(width) * height / maxWidth) + 1) == 0 && world.getTile(getPosition().addX(1)).isReplacable())
    ...
    public void growTree1(World world) {
        Random rnd = new Random();
        if (!grown) {
            grown = true;
            if (height <= maxHeight && (width == 0 || rnd.nextInt(5) == 0) && world.getTile(getPosition().addY(-1)).isReplacable())
