...
public abstract class SubtileOldFluid extends Subtile {
    private float maxHeight = 10, height = maxHeight;
    private boolean dead = false;

    public SubtileOldFluid(Tile t) {
        super(t);
    }
    ...
    public SubtileOldFluid setHeight(float height) {
        this.height = height;
        return this;
    }
    ...
    public float flow(World world, Facing direction, float amount) {
        SubtileOldFluid fluid = getFluid(world, direction);
    ...
    public SubtileOldFluid getFluid(World world, Facing direction) {
        Tile tile = world.getTile(this.tile.getPosition().add(direction));
        if (tile.isWaterproof()) return null;
        SubtileOldFluid fluid = (SubtileOldFluid) tile.getSubtile(getClass(), false);
        if (fluid == null) {
    ...
    public abstract SubtileOldFluid getNewFluid(Tile tile);
